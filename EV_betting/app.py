...
def format_odds_display(odds):
    """Helper function to format odds with a +/- sign."""
    try:
        odds = int(odds)
...
# --- MODIFICATION: Updated find_arbitrage_opportunities function ---
def find_arbitrage_opportunities(props_df):
    ...
    for _, group in grouped:
        ...
        if (prob_over + prob_under) < 1.0:
            ...
            # Append opportunity with new odds formatting
            opportunities.append({
                'player_name': group['player_name'].iloc[0], 
                'prop_type': prop_type_display,
                'line': group['line'].iloc[0],
                'bet_on_over': {
                    'sportsbook': best_over_row['sportsbook'], 
                    'odds': format_odds_display(best_over_row['over_odds']) # Use formatter
                },
                'bet_on_under': {
                    'sportsbook': best_under_row['sportsbook'], 
                    'odds': format_odds_display(best_under_row['under_odds']) # Use formatter
                },
...
def parse_prop_type(prop_string: str) -> dict:
    ...
    qualifiers = {
        ' - 1st Half': '1st Half', ' - 1H': '1st Half', ' - 1st Quarter': '1st Quarter', ' - 1Q': '1st Quarter',
...
def get_combined_data():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__)); data_dir = os.path.join(script_dir, '..', 'nfl_data')
    prop_files = { 'fanduel': 'fanduel_nfl_week_6_props.csv', 'draftkings': 'draftkings_nfl_week_6_props.csv' }
    
    if not os.path.isdir(data_dir): return None, output_structure, f"Error: Data directory not found at '{os.path.abspath(data_dir)}'.", week_number, sportsbooks
    ...
    if fanduel_df.empty and draftkings_df.empty: return None, output_structure, f"No prop data files found in '{os.path.abspath(data_dir)}'.", week_number, sportsbooks
    if not fanduel_df.empty: fanduel_df['sportsbook'] = 'Fanduel'
    if not draftkings_df.empty:
        draftkings_df.rename(columns={'player': 'player_name'}, inplace=True); draftkings_df['sportsbook'] = 'Draftkings'
        if not fanduel_df.empty:
            ...
            def clean_dk_row(row):
                if row['player_name'] not in known_players:
                    full_prop_string = str(row['player_name']); row['player_name'] = extract_player_name(full_prop_string, known_players); row['prop_type'] = full_prop_string
                return row
    ...
    for col in ['over_odds', 'under_odds']:
        if col in props_df.columns: props_df[col] = pd.to_numeric(props_df[col].astype(str).str.replace('−', '-'), errors='coerce')
    props_df.dropna(subset=['game', 'player_name', 'over_odds', 'under_odds', 'prop_type'], inplace=True)
    
    prop_details = props_df.apply(lambda row: parse_prop_type(str(row['prop_type'])[len(str(row['player_name'])):].strip() if str(row['prop_type']).lower().startswith(str(row['player_name']).lower()) else str(row['prop_type'])), axis=1)
    props_df['prop_main'] = prop_details.apply(lambda x: x['main']); props_df['prop_qualifier'] = prop_details.apply(lambda x: x['qualifier'])
    props_df['game_norm'] = props_df['game'].astype(str).apply(lambda g: normalize_game_name(g, TEAM_MAP)); props_df['player_name_norm'] = props_df['player_name'].apply(normalize_player_name)
    ...
    player_team_map = {}; has_team_info_df = props_df[props_df['team_name'] != ''].copy()
    if not has_team_info_df.empty:
        player_team_map = has_team_info_df.drop_duplicates(subset=['player_name_norm'], keep='last').set_index('player_name_norm')[['team_name', 'team_logo']].to_dict('index')
    
    missing_team_mask = props_df['team_name'] == ''
    if missing_team_mask.any():
        props_df.loc[missing_team_mask, 'team_name'] = props_df[missing_team_mask].apply(lambda row: player_team_map.get(row['player_name_norm'], {}).get('team_name', ''), axis=1)
        props_df.loc[missing_team_mask, 'team_logo'] = props_df[missing_team_mask].apply(lambda row: player_team_map.get(row['player_name_norm'], {}).get('logo', ''), axis=1)

    sportsbooks = sorted(props_df['sportsbook'].unique()); props_df['grouping_team'] = props_df['team_name'].replace('', 'Unknown')
    ...
    for (game, team, player, prop_main, prop_qualifier), group in grouped:
        ...
        if team not in output_structure[game]['teams']: output_structure[game]['teams'][team] = {'logo': team_logo, 'players': {}}
        player_props = output_structure[game]['teams'][team]['players'].setdefault(player, {'props': {}})
        market_data = {row['sportsbook']: {'line': row['line'], 'over': format_odds_display(row['over_odds']), 'under': format_odds_display(row['under_odds'])} for _, row in group.iterrows()}
