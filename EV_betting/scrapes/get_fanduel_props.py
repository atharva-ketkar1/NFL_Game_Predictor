...
def get_player_props(event_id, prop_tab):
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36', 'x-sportsbook-region': 'OH'}
    try:
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        return response.json()
...
def get_upcoming_nfl_games(main_page_data):
    ...
    for row in game_rows:
        ...
        if not event_detail: continue
        
        open_time_str = event_detail.get('openTime')
        if open_time_str:
            if open_time_str.endswith('Z'): open_time_str = open_time_str[:-1] + '+00:00'
            try:
...
def main():
    try:
        ...
        print("Invalid input. Please enter a whole number."); return

    print("\nFetching all upcoming NFL games...")
    main_page_data = get_nfl_main_page_data()
    if not main_page_data:
        print("Could not fetch main page data. Exiting."); return
    ...
    for event, market_ids in upcoming_events:
        ...
        if ' @ ' in game_name:
            game_line.update({'away_team': game_name.split(' @ ')[0], 'home_team': game_name.split(' @ ')[1]})
        
        for market_id in market_ids:
            market = markets_data.get(str(market_id))
            if not market or len(market.get('runners', [])) != 2: continue
            
            market_name = market.get('marketName')
            runners = market.get('runners', [])
            if market_name == 'Spread':
                game_line.update({'away_spread_line': runners[0].get('handicap'), 'away_spread_odds': runners[0].get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds'), 'home_spread_line': runners[1].get('handicap'), 'home_spread_odds': runners[1].get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds')})
            elif market_name == 'Moneyline':
                game_line.update({'away_moneyline': runners[0].get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds'), 'home_moneyline': runners[1].get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds')})
            elif market_name == 'Total Match Points':
                game_line.update({'total_line': runners[0].get('handicap'), 'over_odds': runners[0].get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds'), 'under_odds': runners[1].get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds')})
        ...
        for tab_key in ["passing-props", "receiving-props", "rushing-props"]:
            ...
            if not prop_data or 'attachments' not in prop_data or 'markets' not in prop_data['attachments']: continue
            for market in prop_data['attachments']['markets'].values():
                if " - " not in market.get('marketName', ''): continue
                ...
                all_props_data.append({'week': week_number, 'game': game_name, 'player_name': player_name, 'team_name': extract_team_name_from_logo(logo_url), 'team_logo': logo_url, 'prop_type': prop_type, 'line': over_runner.get('handicap'), 'over_odds': over_runner.get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds'), 'under_odds': under_runner.get('winRunnerOdds',{}).get('americanDisplayOdds',{}).get('americanOdds'), 'sportsbook': 'FanDuel'})
            time.sleep(0.5)

    output_dir = 'nfl_data'
    os.makedirs(output_dir, exist_ok=True)
    if all_props_data:
        props_file = os.path.join(output_dir, f'fanduel_nfl_week_{week_number}_props.csv')
        print(f"\nWriting {len(all_props_data)} props to {props_file}...")
        with open(props_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=all_props_data[0].keys()); writer.writeheader(); writer.writerows(all_props_data)
    if all_game_lines_data:
        lines_file = os.path.join(output_dir, f'fanduel_nfl_week_{week_number}_game_lines.csv')
        print(f"Writing {len(all_game_lines_data)} game lines to {lines_file}...")
        with open(lines_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=all_game_lines_data[0].keys()); writer.writeheader(); writer.writerows(all_game_lines_data)
    print("\nScraping complete.")
